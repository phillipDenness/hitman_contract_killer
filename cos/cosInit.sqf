...
blackListTowns = ["sagonisi","kavala","aggelochori"];// Remove towns from COS

whiteListMkrs=["kavalamkr"];// Add Custom Markers for COS to populate 
...
_slack=5;// Additional spawn points
...
_randomisation=10;
	...
	_parked=6;// Parked Vehicles Spawned
 	...
 	if (_houses > 140) 
		then {
	_civilians=35+ round(random _randomisation);// Civilians spawned
	_vehicles=1;// Moving Vehicles Spawned
	_parked=10;// Parked Vehicles Spawned
			};
...
//null=[1,350,cosMkrArray,[0,1],[2,1],[0,1],[0,1]]call eos_Init;
