...
_information=server getvariable _popVar;
	...
	_count=_parked + _vehicles + _civilians; 
...
waituntil {!populating_COS and !populating_EOS};
populating_COS=true;
_glbGrps=server getvariable "cosGrpCount";

//SPAWN CHECKPOINTS
for "_i" from _count to (_count+1) do {
null=[_mkr,_roadPosArray,_i] execVM "cos\checkPoint.sqf";
						};
...
if (!(server getvariable _trigID)) exitwith {_isActive=false;};
		...
		_direction = [_tempPos, _connectedRoad] call BIS_fnc_DirTo;
			...
			_veh = createVehicle [_tempVeh, _tempPos, [], 0, "NONE"];
			_veh setdir _direction;
			_veh setPos [(getPos _veh select 0)-4, getPos _veh select 1, getPos _veh select 2];
